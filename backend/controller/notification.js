...
const notification = (req, res) => {
    ...
    let c = 0;
    const {name,surname,email,email_post_owner,image} = req.body; 
    let message = "";

    console.log(email_post_owner)

    message = name +" "+surname +" has liked your photo"
    ...
    if(email && id){
     ...
     pool.query('select * from notification where email = $1 And post_id = $2' ,[email,id],(error, results)=> {
       ...
       if (results.rowCount > 0) {
        ...
        pool.query('INSERT INTO notification (post_id,name,surname,message,email,email_post_owner,image) VALUES ($1,$2,$3,$4,$5,$6,$7)',[id,name,surname,message,email,email_post_owner,image],(error, results) => 
              {
